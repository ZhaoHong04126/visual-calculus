...
export default function LimitLaws() {
  return (
    <section className="subsection mt-12">
      <h2 className="section-title text-2xl font-bold mb-4 border-b-2 border-emerald-500 pb-2">極限的運算法則與夾擠定理</h2>
      ...
      <div className="math-box mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 bg-gray-50 p-4 rounded text-sm">
          <div><strong>1.1 和/差：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [f(x) \pm g(x)] = \lim_{x \to a} f(x) \pm \lim_{x \to a} g(x)" /></div>
          <div><strong>1.2 常數積：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [c \cdot f(x)] = c \cdot \lim_{x \to a} f(x)" /></div>
          <div><strong>1.3 乘積：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [f(x) \cdot g(x)] = \lim_{x \to a} f(x) \cdot \lim_{x \to a} g(x)" /></div>
          <div><strong>1.4 商：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} \frac{f(x)}{g(x)} = \frac{\lim_{x \to a} f(x)}{\lim_{x \to a} g(x)} \quad (\text{前提：分母極限不為 0})" /></div>
          <div><strong>1.5 次方：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [f(x)]^n = [\lim_{x \to a} f(x)]^n" /></div>
          <div><strong>1.6 開根號：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} \sqrt[n]{f(x)} = \sqrt[n]{\lim_{x \to a} f(x)}" /></div>
        </div>
      </div>
      ...
      <div className="concept-grid mt-6">
        ...
        <div className="concept-card" style={{ borderLeft: '4px solid #ef4444', backgroundColor: '#fef2f2' }}>
          <h3 className="text-lg font-bold mb-2">
            3. 處理 <InlineMath math="\frac{0}{0}" /> 不定型
          </h3>
