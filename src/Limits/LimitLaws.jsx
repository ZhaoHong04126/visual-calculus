    ...
    <section className="subsection mt-12">
      ...
      <div className="math-box mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 bg-gray-50 p-4 rounded text-sm">
          <div><strong>2.1.1 和/差：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [f(x) \pm g(x)] = \lim_{x \to a} f(x) \pm \lim_{x \to a} g(x)" /></div>
          <div><strong>2.1.2 常數積：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [c \cdot f(x)] = c \cdot \lim_{x \to a} f(x)" /></div>
          <div><strong>2.1.3 乘積：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [f(x) \cdot g(x)] = \lim_{x \to a} f(x) \cdot \lim_{x \to a} g(x)" /></div>
          <div><strong>2.1.4 商：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} \frac{f(x)}{g(x)} = \frac{\lim_{x \to a} f(x)}{\lim_{x \to a} g(x)} \quad (\text{前提：分母極限不為 0})" /></div>
          <div><strong>2.1.4 次方：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} [f(x)]^n = [\lim_{x \to a} f(x)]^n" /></div>
          <div><strong>2.1.5 開根號：</strong><BlockMath settings={{ strict: false }} math="\lim_{x \to a} \sqrt[n]{f(x)} = \sqrt[n]{\lim_{x \to a} f(x)}" /></div>
        </div>
      </div>
